...
ABBR = {'сек', 'мин', 'доп', 'маг', 'физ', 'чист', 'макс', 'скор', 'движ',
        'атаки', 'реген', 'баз', 'ед', 'др', 'ср', 'см', 'нейтр', 'здан'}
...
def check(base_dir, bundle_dir):
    ...
    for stem in STEMS:
        ...
        for key, eng in base.items():
            ...
            # \b не срабатывает после цветового кода: в "^oсек.у" между
            # латинской o и кириллицей нет границы слова
            if re.search(r'(?<![А-Яа-яЁё])сек\.[а-яё]', value):
            ...
            # Исходный проект (bd4874b) вырезал "вы можете" по всему
            # переводу, оставив предложения со строчного инфинитива: "Вы
            # уже в матче. получить совпадение...". После сокращений ("на 8
            # сек. усилить") строчная буква законна, как и существительные
            # на -сть ("доп. способность")
            stub = [m.group(2) for m in re.finditer(
                        r'(?:([A-Za-zА-Яа-яЁё0-9]+)\.|[!?])\s*'
                        r'([а-яё]+(?:ть|ться|ти|чь))\b', bare)
                    if not (m.group(1) and m.group(1) in ABBR)
                    and not m.group(2).endswith('сть') and m.group(2) != 'вплоть']
            if stub:
                add('предложение с инфинитива (вырезано "вы можете"?)',
                    '%s:%s' % (stem, key), ', '.join(stub[:3]))
